...
## 할것
* 페이징 조건(동적쿼리 + no offset) 문서화(id이 외에 다른 조건을 더해서 정렬을 진행할때에는 프론트에서 lastid를 분별하는 코드가 필요하다. 이 경우 id를 desc로 정렬할때에는 서버로부터 받은 값들중 가장 작은 id를 lastid로 넘기고, asc로 정렬한다면 가장 큰 id를 lastid로 넘겨주어야한다.)
* 레디스 고민해보기, 아무리생각해도 캐쉬는 쓰긴 써야할 것 같음. 계속 카운쿼리 날리기에는 자원소모가 너무 심함

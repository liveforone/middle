...
광고 노출 : 가격별로 광고노출횟수를 제한한다(50, 100, 150,,,,1000)
...
## 널체크
에러를 최소화 하기 위해서 매퍼나 서킷 브레이커 사용시 빈 객체를 리턴하고
널체크시에는 빈 객체의 id를 가지고, id가 없을 경우에는 대표 컬럼을 가지고 널 체크를 한다.
...
코드 요구사항에 위의 내용포함 스타일 가이드 내용 제목만 해서 필요한것 간단히 작성

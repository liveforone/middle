...
* <u>추천 알고리즘</u>
* <u>코드로 푼 동시성 문제</u>
* 라는 3가지 키워드를 집중적으로 고민하고 다룬 프로젝트 입니다.
...
상점 서비스
상점 추천 서비스
상점예약 서비스(상점에서 등록한 예약과, 그 예약들의 예약가능자 수를 저장하는 두개의 테이블을 저장함)
...
## 할 것
* shop은 시간을 나누듯 지역을 크게 나누어 따로따로 저장해야 검색시 편리해진다.
* 리파지토리 만들고 컨슈머 완성하기
* 리스트 조회시 첫 페이지에 추천 상점 넣기
* 검색 쿼리 참조해서 만들기
* 모든것은 username을 기본 식별자로 사용한다.
* 상점 서비스 kafka로 회원 탈퇴 정보 컨슈머로 받으면 상점 삭제-> 상점 등록 예약 삭제 요청
